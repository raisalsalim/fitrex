...
export function getProfiles() {
  const defaultList = [
    { id: 'profile_1', name: 'Raisal', color: '#10b981', avatar: 'R', isDefault: true, createdAt: Date.now() }
  ];
  return safeParse(STORAGE_KEYS.PROFILES, defaultList);
}
...
export function getActiveProfileId() {
  ...
  const def = profiles[0]?.id || 'profile_1';
...
export function getPersonalRecords(profileId = getActiveProfileId()) {
  ...
  workouts.forEach(workout => {
    (workout.exercises || []).forEach(ex => {
      const name = ex.name;
      if (!prMap[name]) {
        prMap[name] = { maxWeight: 0, maxVolume: 0, best1RM: 0, date: workout.date };
      }
      (ex.sets || []).forEach(set => {
...
export function getBodyMetrics(profileId = getActiveProfileId()) {
  const metrics = safeParse(`${STORAGE_KEYS.BODY_PREFIX}${profileId}`, []);
...
export async function syncWithGist(forcePush = false) {
  ...
  try {
    ...
    // Step 1: If gistId is not set, try to find an existing Fitrex gist or create a new one
    if (!gistId) {
    ...
    // Step 2: Fetch current content from Gist
    ...
    // Step 3: Write merged local data back to Gist
